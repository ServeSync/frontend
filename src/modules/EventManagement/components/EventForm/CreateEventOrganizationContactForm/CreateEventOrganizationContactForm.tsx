    ...
    <div className='border-[1px] border-gray-300 p-6 rounded-xl'>
      ...
      <div className='grid grid-cols-5 gap-6'>
        <div className='col-span-1 relative bg-slate-300 rounded-full outline-none w-full pt-[100%]'>
          <img
            src={eventOrganization.imageUrl}
            alt='avatar_organizer'
            className='rounded-xl object-cover w-full top-0 h-full object-top absolute '
          />
        </div>
        ...
        <thead className='bg-[#edeeef] border-[1px] border-gray-200'>
          <tr className='text-[14px] text-gray-600'>
            {!event?.hasOrganizedRegistration || event?.hasOrganizedRegistration === undefined
              ...
              : EventOrganizationTableHeader.slice(0, -1).map((item) => (
                  <th className='px-2 py-2 font-semibold' key={item.id}>
                    <span>{item.name}</span>
                  </th>
                ))}
