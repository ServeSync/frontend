...
const CircleChart = ({ educationProgramResult, isLoading }: Props) => {
  const chartData = {
    datasets: [
      {
        data: [educationProgramResult?.gainScore, educationProgramResult?.requiredActivityScore - educationProgramResult?.gainScore],
  ...
  const drawMiddleTextPlugin = {
    ...
    beforeDraw: (chart: any) => {
      ...
      ctx.fillText(`${(educationProgramResult?.gainScore * 100 / educationProgramResult?.requiredActivityScore).toFixed(1)}%`, xCoor, yCoor);
      ctx.restore();
    },
  }
    ...
    <Fragment>
      <div className='flex items-center'>
        <div className='col-span-1'>
          {isLoading ? <Skeleton /> : <Doughnut data={chartData} options={{ maintainAspectRatio: false, cutout: '70%', responsive: true }} plugins={[drawMiddleTextPlugin]}/>}
        </div>
